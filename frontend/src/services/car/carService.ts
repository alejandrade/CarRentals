...
class CarService {
     ...
     * @returns - A promise with the paginated response of cars.
     */
    async fetchAllAvailableCars(page: number = 0, size: number = 10): Promise<PaginatedCarResponse> {
        const response = await authFetch(`${this.BASE_URL}/staff/v1/car/available?page=${page}&size=${size}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            },
